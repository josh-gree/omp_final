...
void loop2(void) {
  ...
  rN2 = 1.0 / (double) (N*N);  
#pragma omp parallel for num_threads(N_Threads) schedule(dynamic,16) default(none) shared(b,c,jmax) private(i,j,k) firstprivate(rN2)
  for (i=0; i<N; i++){ 
    for (j=0; j < jmax[i]; j++){
      for (k=0; k<j; k++){ 
