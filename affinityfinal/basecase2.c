...
int main(){
  ...
  for (i =0;i<2;i++){
  #pragma omp parallel num_threads(N_Threads) shared(listl1,a,b,bounds)
    {
 ...
 double t1l1= omp_get_wtime();  
  ...
  for (i =0;i<2;i++){
  #pragma omp parallel num_threads(N_Threads) shared(listl2,a,b,c,jmax,bounds)
    {
